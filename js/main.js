...
function initCursor() {
  ...
  const ORB_R = 5;
  const LERP = 0.18;
  const R = 230, G = 84, B = 45; // ember red
  ...
  function draw() {
    ...
    if (trail.length > 3) {
      [
        { width: ORB_R * 10, opacity: 0.03 },
        { width: ORB_R * 5, opacity: 0.05 },
        { width: ORB_R * 1.5, opacity: 0.08 },
    ...
    const glowR = ORB_R + pulse * 2;

    const outerGlow = ctx.createRadialGradient(orb.x, orb.y, 0, orb.x, orb.y, glowR * 5);
    outerGlow.addColorStop(0, `rgba(${R},${G},${B},${0.3 + pulse * 0.1})`);
    outerGlow.addColorStop(0.4, `rgba(${R},${G},${B},0.08)`);
    outerGlow.addColorStop(1, `rgba(${R},${G},${B},0)`);
    ctx.beginPath();
    ctx.arc(orb.x, orb.y, glowR * 5, 0, Math.PI * 2);
    ctx.fillStyle = outerGlow;
    ctx.fill();

    const core = ctx.createRadialGradient(orb.x - ORB_R * 0.3, orb.y - ORB_R * 0.3, 0, orb.x, orb.y, glowR);
    core.addColorStop(0, `rgba(255,220,120,1)`);
    core.addColorStop(0.4, `rgba(${R},${G},${B},0.95)`);
    core.addColorStop(1, `rgba(${Math.round(R*0.8)},${Math.round(G*0.8)},${Math.round(B*0.8)},0.8)`);
    ctx.beginPath();
